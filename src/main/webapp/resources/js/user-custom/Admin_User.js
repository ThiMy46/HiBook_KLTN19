$(document).ready(function getlistuser() {
	...
	var role1='<select class="badge-info myselect"> <option data-id="1" value="USER" selected>USER</option>'+
	  '<option data-id="2" value="ADMIN">STAFF</option><option data-id="3" value="ADMIN">ADMIN</option></select>';
	var role2='<select class="badge-warning myselect"> <option data-id="1" value="USER" >USER</option>'+
	  '<option data-id="2" value="ADMIN" selected>STAFF</option><option data-id="3" value="ADMIN">ADMIN</option></select>';
	var role3='<select class="badge-danger myselect"> <option data-id="1" value="USER" >USER</option>'+
	  '<option data-id="2" value="ADMIN" >STAFF</option><option data-id="3" value="ADMIN" selected>ADMIN</option></select>';
    $.ajax({
    	type : "GET",
    	url : "api/user"
    }).then(function(data) {
    ...
    $('#table-user tbody').on('change', 'select.myselect', function () {
		...
		if(idRole==1){
    		...
    		this_role.append(role3);
    	}
		
        $('#change-role').click(function(){
    		$('#modalChange').modal('toggle');
    		$.ajax({
            	url : "api/user/"+idUser+"/role/"+idRole,
            	type : "PUT",
