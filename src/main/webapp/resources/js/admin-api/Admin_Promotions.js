    ...
    $.ajax({
    	...
    	$.each(data, function (i, item) {	 	
    		...
    		auth+='<tr><td class="idBook">'+item.idPromotion+'</td> ' +
                '<td ><a href="#" style="text-decoration: none;" class="namebook font-weight-bold">'+item.contentPromotion+'</a></td>'+
                '<td class="picBook"><img class="card-img-top" src="resources/images/banner/'+item.picPromotion+'"/></td>'+
                '<td class="publicationDate">'+item.saleOff +'%'+'</td>'+
                '<td class="quantity">'+item.timeEnd+'</td>'+
                '<td class="price">'+item.timeStart+'</td>'+
                '<td class="discount">'+item.titlePromotion+'</td>'+
